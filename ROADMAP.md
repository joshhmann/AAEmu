...
- **Q7 — PB-005 named grounding decisions (rulings first, then build).**
  ...
  - Phase 1 landed 2026-09-06 (dry-run telemetry, empty deck store; P1 registry hardening alongside): d6655585f + faa6a9c89. Full suite 2879/0/1 x2. Phase 2 (writes) needs second approval. LevelingLoop Stage4 quest-data vector filed as separate ticket (not fixed here).
...
- FINAL Tier-3 exit conjunction = (a) SHAPE + (b1) + (b2)
  ...
  - (b2) business-state progression: harvest/travel timers actually advancing (before/due/after/restart-conservation) — asserted NOWHERE in `:296-449`; planned, not evidenced. Update 2026-09-06: since asserted — b2 timer-state probe (`A5Tier3TimerProgression.cs`: plant/canary setup, per-sample harvest/travel reads, end `ValidateTimerProgression`, bounded `Probe_A5Tier3RestartConservesDormantTimers`) PASS 2026-09-06 at tested revision 02903804b; definition above is the historical pre-assertion shape.
...
- A `passed=true` report closes (b1) only; FULL Tier-3 additionally needs (a) re-shown at the same tip and (b2) asserted. Explicit A5 work: A5-W1 collect the (b1) report at the pinned tip (readiness: DONE — soak #2 PASS 2026-09-05); A5-W2 design + run the (b2) timer-state assertion (readiness: DONE (b2 asserted soak PASS 2026-09-06 + bounded restart PASS 2026-09-05))
...
- Soak #2 was the (b1) candidate (CLOSED-PASS 2026-09-05); the b2 asserted soak 2026-09-06 is the comparable post-change zero-breach run (passed:true, failures:[] at 02903804b). A5 stays OPEN pending only (a) SHAPE re-show.
- Update 2026-09-08: A5 CLOSED. (a) SHAPE re-shown 4/4 at engine `a23cdc33e` (report runAtUtc 2026-09-08T08:43:32Z, isolated lane): 1000/1000 registered, exactly 50 embodied (50/50 dematerialized on leave), RSS Δ -12.9% (6204.9 → 5404.7 MB medians, within ±15%), wake p95 256.2 ms. En-route fixes: scheduler DI deadlock `a23cdc33e` (engine; `b199e5d30` had closed the BotRoamStepExecutor → BotGoalArbiter → SchedulePhase → BotScheduleService cycle — first scheduler resolve wedged forever, metrics proof tick.available=true post-fix) and harness stale-pid FindGamePid guard `9b790f69f` (test-only; arm-T RSS had read -1). (b1) soak #2 PASS 2026-09-05 (tested binary `322390b32`) and (b2) asserted soak PASS 2026-09-06 (`02903804b`) + bounded restart PASS stand as the historical legs — the conjunction is met with b1/b2 evidenced at their tested revisions per the letter above. H separate; "preferably 12-hour" stays recommendation-only.
...
population isolation `c97909f4f`, and opt-in six-hour leg `155c82c66` remain
integrated.
...
- **PB-002 remains PARTIAL / configurable support, not universal closure.**
  `QuestActObjMateLevel` has a configurable `MateLeg`; this is not universal
  closure across canonical carriers or progression routes.
- `MateLeg` uses explicit `LoopOptions.MateGrowthItemId`; default `0` fails
  closed. It uses the ordinary item-use path and never writes mate XP, level,
  or the quest objective directly.
...
- Focused evidence is `GameplayActorMateLevelRigTests` **5/5** and
  ...
  report **0 errors**. This is deterministic rig/proxy evidence only: no live
...
- **PB-002 remains PARTIAL; broad autonomous progression is OPEN.** Evidence is
  deterministic rig/proxy only, not live authenticated progression or
  human/client evidence.
- Landed objective families are interaction, item-use, item-group use/gather,
...
- Remaining objective gaps (2026-08-30 research decision):
  ...
  **implementable after a narrow rig proof** — 10 raw rows, 6 live carrier
  ...
  support.
- Focused results: `LevelingLoopScenarioRigTests` 32/32,
  `QuestActObjAggroTests` 2/2, `QuestEtcItemObtainRigTests` 3/3,
  `QuestZoneKillVictimRigTests` 2/2, `PvpFlaggingRigTests` 11/11; Game and
  UnitTests Release builds report 0 errors.
...
**2026-08-30 landed-state correction:** CompleteQuest and Level pursuit are
committed on current `develop`; CheckTimer and SupplyRemoveItem are classified
as non-objective gate/cleanup acts. The former uncommitted/local wording and
gap lists are historical/pre-fix only. Current objective gaps are Ability
(11 quests) and Mate (7 live quests; 1 orphaned data row), with Aggro still
partial at its resolvability boundary. Item-group, ZoneKill, and EtcItemObtain
are landed objective families, not remaining gaps. Broad PB-002 autonomy remains
OPEN and current evidence is rig/proxy only.
...
**Historical/pre-fix census record:** the older runnability snapshot listed
ObjZoneKill, ObjAggro, ObjCompleteQuest, and EtcItemObtain as unsupported act
families. That dated list is retained for provenance and is not the current
PB-002 objective gap list.
...
`792774d7707b8b578b8d9975896e0a1ac719f361` is **2496 total / 2495 passed /
0 failed / 1 skipped**; compiler check **0/0**; MCP stdio smoke **39 tools**.
Command: `./scripts/gate.sh`. The sole skip is
...
passed 2/2. Do not substitute another full-gate count.
...
action, A5 acceptance run):**
  ...
  **UPDATE 2026-08-27 (behavioral gate evidence baseline
  ...
  `792774d7707b8b578b8d9975896e0a1ac719f361`; source commit `b230bd8a2`):**
...
`792774d7707b8b578b8d9975896e0a1ac719f361`; behavioral gate evidence remains
pinned to baseline `3871459d142fdd1767b9365a1de8d4cd3652ab0e` from a normal
clone; per-run soak ownership hardening `799b698ad` is source/test evidence):**
