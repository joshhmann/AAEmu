...
| M3a Homestead shell | Closed on scripted-actor/fixture proxy evidence only; the ordinary player loop is place/build → plant/harvest → storage/coffer/furniture state → observable ownership/contents result. The current property replay remains ordered stages, not autonomous decision selection; M3b restart persistence is a separate loop. Current source/test HEAD `a77ef878d8fcba297c32c0228e712e0695cc4887` includes source commit `1a3f13dc1` and `HousingStorageFurnitureTests` 13/13. Player/H UAT remains open; no live-client claim. |
| M3b Property persistence | Closed on restart/crash engineering evidence, separately from the M3a player loop. The M3 focused aggregate was 178/178 at prior exact source/test baseline `b9a72825f`; named slices were M3a exit 1/1, M3b furniture 4/4, phase restart 10/10, property policy 11/11, and repair scanner 13/13. The current permission-fix source/test HEAD is `a77ef878d8fcba297c32c0228e712e0695cc4887`; `HousingStorageFurnitureTests` is 13/13. Bot/property replay is scripted/fixture proxy, not autonomous or live-client evidence; Player/H UAT remains open. |
