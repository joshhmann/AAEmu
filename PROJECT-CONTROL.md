...
- **Target:** ArcheAge 1.2 client and reference data `compact.sqlite3` r208022
  (read-only)
**Audited source/evidence baseline:** `0ce518ac03a18de00fff1516aa9e794e8566bee6`
(`develop`, local source/test HEAD; M5 proposal `263ecc66c474ca1c5f4b085e86ef3e47f49fd1`
and M6 cancellation `950cfd279` are in its ancestry).

The M5 bounded decision primitive is integrated in `LevelingLoop`'s accept
choice: `BotDecisionProposal`, `BotDecisionSelector`, and `BotDecisionCycle`
carry immutable observed context, enforce hard legality before preference, bound
candidate sets, select deterministically by fixed priority/personality/tie-break,
and require a terminal postcondition before existing `GameplayActor` dispatch.
The focused proposal contract is **5/5**. This is a decision primitive plus a
scoped quest consumer, not universal bot autonomy; broad M5 policy remains open.

The M6 cancellation boundary adds `BotDriveClient.CallAsync` token/timeout
support while preserving synchronous `Call`; A5 and A5Tier3 seed bridge calls
are async, and Tier3 workers share cooperative cancellation/deadline propagation
without `Thread.Abort`. Focused
results are BotDriveClientCancellationTests **3/3**, SoakOwnershipTests **2/2**,
and BotPresenceCoordinator **13/13**. No six-hour soak exists; no M6 full-exit
or H/UAT claim is inferred.

The current full gate is **2504 total / 2503 passed / 0 failed / 1 skipped**,
compiler **0/0**, MCP **39 tools**. The sole skip is
`Provision_Activate_Persist_Deactivate_RoundTrip`, requiring
`AAEMU_LIVE_RIG=1` and `AAEMU_E2E_DB_PASSWORD`. Existing reports and source/test
SHA boundaries remain historical where stated.
...
| M6 Deterministic playerbot framework | Current source/test HEAD `0ce518ac03a18de00fff1516aa9e794e8566bee6`; cancellation commit `950cfd279` adds async token/timeout bridge calls and cooperative A5Tier3 worker deadline/stop while retaining sync `Call`; BotDriveClientCancellationTests 3/3, SoakOwnershipTests 2/2, BotPresenceCoordinator 13/13. No six-hour soak, M6 full exit, or H/UAT claim. |
...
| **M6 human/exit gate — B4 restart** | **N/A (restart gate)** | **Unknown/Open (harness-driven lifecycle; autonomous decision closure not demonstrated)** | **A/R:** current M6 focused evidence is **105/105** at `0ce518ac03a18de00fff1516aa9e794e8566bee6`; cancellation `950cfd279` adds token/timeout-aware `BotDriveClient.CallAsync` with compatible sync `Call` and cooperative A5Tier3 worker cancellation/deadline propagation without `Thread.Abort`; BotDriveClientCancellationTests 3/3, SoakOwnershipTests 2/2, BotPresenceCoordinator 13/13. No six-hour soak or M6 full-exit result. |
