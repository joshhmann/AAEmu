...
> Quest-runnability (M2a/M2c + WI-2 + WI-3 + WI-4, band 1-30 census 2026-08-09, post-drop on merged develop): **2085/2085 driven quests runnable, 0 FAIL** across the
> 2096-quest scenario-harness census (117 M2a-dropped quests excluded — 26 engine-stuck + 91 shells). **Band 1-10: 560 PASS / 0 SKIP / 0 FAIL = 100.0% PASS-or-doc-SKIP** of
...
> All SKIPs documented-SKIP with reason (11): 8 orphaned contexts (no quest_contexts row),
> 2 unsupported-act-type (CompleteQuest 2 — the WI-2
...
> context, excluded from t11 so the census gains no new orphan SKIP), 6069 remains SKIP for let-it-done-without-report-act, an engine completion-path class), plus 1 let-it-done-without-report-act (6069). Band-21-30 sweep calibration: kind_id-1 None components (legacy task-board
...
> carriers → 6/6 driven (114 cumulative); zero PASS→SKIP regressions.
