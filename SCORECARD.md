...
| PVP-01 | Flagging, factions, damage, honor, death/recovery | Later | 2 | U | U | U | U | U | **2026-08-25 (mechanics/pvp-domain.md): C=2.** CanAttack resolution order reconstructed (null fail-open → zone/mother-faction shield → Retribution-flag target → ForceAttack attacker → ZONE-01 peace block → Hostile fallback); honor formulas verified (Conflict solo 10 / War solo 20 + 4/assist, 30 s assist windows, War victim −10 clamp ≥0, PvpHonorRate config); faction storage verified (system_factions/_relations + MySQL characters.faction_id); pirate punishment conversion engine-wired. W stays U until the flagged-aggression handshake runs live E2E (slice-1 spec'd, zero source changes); voluntary immigration = log-only stubs; known wart: missing mother-relation row defaults Friendly. H=UNKNOWN. **2026-08-26 (wave 7) MAJOR FINDING + PARTIAL FIX:** flagged-aggression composed flow FAILED live — handshake + target acquisition passed but ZERO damage applied; caught by the new PvpHandshakeE2eTests. Root cause: buff 2423 "LoggedOn" grants ~20 s full damage-immunity at every login (spawn-camp protection) and DamageEffect's immune early-return skipped HP loss AND the crime branch. Fixes: crime branch extracted to `RegisterCrimeForAttempt` and called even on immune early-return (aggressive acts register evidence even when damage is immune); Skill apply-loop exceptions now logged+rethrown (TaskManager silently discarded them, masking failures); E2E hardened to wait out the protection window. Post-fix: real damage lands, bloodstain spawns, crime chain executes; ZONE-01 Peace enforcement + homeland mother-shield LIVE-verified in the same runs. RESIDUAL: Retribution 2167 wire-observable on immune hits (suspect AddBuff stack-rule silent return — follow-up probe documented). Grades stay conservative pending the next audit pass |
