...
| COMBAT-01 | PvE combat, death, resurrection, loot | M2/M5 | U | 1 | U | U | U | U | `SkillManager`; combat audit. **2026-09-13 LIVE recovery-leg PASS:** `RecoveryHealingE2eTests` — honest wound (ReduceCurrentHp) 720→1, potion 8515 (L10, skill 11715, fixed heal 990 = heal_effects row 78) through the real `GameplayActor.UseItem` contract path restores 1→991 (+990); additive mail `wound`/`use`/`stock` bridge ops + charState hp/maxHp observability; report `/root/aaemu-e2e/logs/recovery-heal-report.json`. No grade change (single recovery leg); H=UNKNOWN. **OPEN-RULING 2026-09-15:** COMBAT-01 W=1 vs REPAIR-01 W=2 grading split needs Josh ruling — no regrade here |
...
| PACK-01 | Craft, carry, place, load, unload, sell trade pack | M4 | 2 | 2 | U | 2 | 2 | 2 | M4-2 (fix/m4-2-trade-packs, t_449d0c41): level-10 craft/sell gates, origin-zone StoreCantSellSameZone, 6-day placed-pack expiry, 22 h mail delay; `SpecialtyManagerTests` 21/21 (sale math, 80/20, coin routes, gates); `M4_2TradePackRestartE2eTests` — plant_time + made_unit_id survive kill -9 (PASS on merged tree 2m12s); `M4ExitIntegratedSessionTests` — full craft→load→travel→unload→sell→reward loop, reward math 91238 base / 124540 payout verified, 2× repeat (4 scripted actors = M5-stand-in). **H=U (reconciled 2026-08-12): exit driven by scripted actors — proxy/bot-functional, H UNKNOWN until Josh runs it; M4 economic/navigation replay = deferred gate** **2026-09-10 B-series (B4):** budgeted S-run PASS — 4 actors x 9.0 min wall / 7.0 min steady-state window x 1 kill-9, GateBudgets thresholds + warmup-blind accounting, 48/48 sibling criteria (report `/root/aaemu-e2e-b4pack/logs/b4-pack-soak-report.json`, measured revision `d38b6beaa`); the run exposed a concurrent pack-scan theft defect fixed in `b3989f8e0`. **S=2.** |
| SLAVE-01 | Cart/ship summon, seats/cargo, cleanup, recovery | M4 | 2 | 2 | U | 2 | 2 | 2 | M4-3 (fix/m4-3-vehicle-lifecycle, t_4a91a4f5, Rei gate t_5019f7b1 PASS): despawn gates owner/312/288/801, BindSlave 324, RidersEscape 640; `SlaveLifecycleTests` 29/29; `M4VehiclesE2eTests` — two kill -9 restarts, row intact, exactly 1 row (PASS on merged tree 3m09s); `M4ExitIntegratedSessionTests` — pack loaded on slave, 801 despawn refusal, unload → despawn allowed (4 scripted actors = M5-stand-in). **H=U (reconciled 2026-08-12): exit driven by scripted actors — proxy/bot-functional, H UNKNOWN until Josh runs it; M4 navigation replay = deferred gate** **2026-09-11 B-series (B4 SLAVE-01 S-run):** budgeted S-run PASS — 4 actors x 9.0 min wall / 7.0 min steady-state window x 2 kill-9 restarts, GateBudgets thresholds + warmup-blind accounting, 48/48 sibling criteria, pack instances distinct per wagon (report `/root/aaemu-e2e-b4slave-r2/logs/b4-slave-soak-report.json`, sha256 `8796d079…`, verdict PASS, measured revision `e2abd29b`). The prior b4slave04 FAIL @VERIFY `pack-conserved` was a stale-binary artifact — the lane DLL was published before pack-scan fix `b3989f8e0`; re-run with `E2E_REBUILD=1` cleared it. **S=2.** |
| TRADE-01 | Direct player-to-player item/currency trade | Later | U | 2 | U | 2 | U | N/A | `TradeManager`; separate from trade packs. **2026-08-23 (d4b5e524c): trade FUNCTIONAL** — OkTrade cancel-then-finish KeyNotFoundException fixed; both-locked AND both-ok gate (was !a && !b single-side exploit); TradeOffer/TradePutup/TradeLockOk contract actions; `TradeHandshakeScenarioRigTests` 5/5. W=2 (real engine path end-to-end); **2026-09-13 LIVE PASS:** `DirectTradeE2eTests` — two provisioned bots, bob converged to 2 m (5 m trade range), potato 7992 ×1 through OFFER→PUTUP→LOCK-ALICE→LOCK-BOB with alice-conserved + bob-received criteria; report `/root/aaemu-e2e/logs/trade-handshake-report.json`. **A=2 (item-handshake live; currency leg open)**; H=UNKNOWN |
| MERCHANT-01 | NPC vendor buy/sell, price, stock, refund/error paths | M2/M4 | U | 2 | U | 2 | 2 | N/A | **2026-08-26 recovery:** MerchantRigTests drive the REAL CSBuyItemsPacket/CSSellItemsPacket paths over capture-backed connections with real NpcManager goods packs; bot Buy/Sell actions replicate the packet seam. Merchant trio is now merged on develop (`cb514c42e` funds gate, `beaf9b82e` buyback refund, `3ba33b3af` grant-failure rollback; merge `e5db6d390`). `EconomyDayCycleE2eTests` live conservation E2E passed, with money/bank/items held across kill -9 restart (`/root/aaemu-e2e/logs/m8-economy-cycle-reconcile.md`). W=2/A=2; **H=U** — no human-client shop run. **2026-09-10 B6 R-run:** `B6MerchantConservationE2eTests.Merchant_BuySell_ConservedAcrossKill9_OnRealPacketPaths` PASS 1/1 in 3m35s on isolated lane (own ports/DB, PID-verified teardown) — wire `CSBuyItemsPacket` buy 4 x seed 15659 @ 25, live insolvent-buy refusal, wire `CSSellItemsPacket` refund 48, save → kill-9 → MySQL money + item rows byte-equal, post-restart rebuy functional (`/root/aaemu-e2e-b6merchant/logs/b6-merchant-conservation-reconcile.md`). Canonical audit: `scorecard-explorations/mechanics/merchant-domain.md`. W=2/A=2/R=2; **H=U**. |
...
| DOMINION-01 | Dominion/castle-siege: zones, settings, plans, phase schedule, tax rate | M9+ | 2 | 2 | U | 2 | 2 | 2 | NEW + promoted 2026-08-26 (`mechanics/dominion-domain.md` dossier): first real reconstruction of the dominion system — DominionManager loads siege_zones(6)/siege_settings(11)/siege_plans(158) from canonical compact data; additive MySQL `aaemu_game.dominions` (SQL/updates/2026-08-26_aaemu_game_dominions.sql + base file); CSUpdateDominionTaxRatePacket wired (policy validation → persist → echo); phase cron Peace→Declare→Warmup→Siege→Payoff announcing via SCSiegeAlertPacket (0xed, documented placeholder payload); DeclareDominion special-effect now persists instead of hardcoded broadcast; enter-world rebroadcast of stored dominions; persistence E2E across kill -9 PASS (branch d42e708f5→66f124533). Combat/siege-battle explicitly NOT implemented (later slices); declare-trigger UI path still UNKNOWN (pack-planting hypothesis from client data); S=U/H=UNKNOWN **2026-09-10 B-series (B5):** curated S-run PASS (report `/root/aaemu-e2e-b5/logs/dominion-soak-cycle-report.json`, verdict PASS, no source-revision field recorded; test landed `463d46d86`) — declare -> tax-rate round-trip -> phase-cron watch -> kill-9 persistence, budgets green, siege combat explicitly descoped. **S=2.** |
...
| CRIME-01 | Crime evidence/points, reporting, persistence | M9 | 2 | 2 | U | 1 | 1 | U | **2026-08-25 (mechanics/justice-domain.md): C=2.** CrimeManager fully wired — evidence doodads (bloodstains 877/878, footprints 3313/3314) → CSReportCrimePacket → CrimePoint/InfamyPoint property-setter threshold checks (Wanted 3710 @ 50, Pirate conversion @ Infamy 3000) → MySQL `crime` table via SaveManager dirty flush; wanted/arrest thresholds re-checked at login; packet table complete (all 12 justice C2G opcodes registered, 21/25 G2C sent by live code); H=UNKNOWN. **2026-08-26 (wave 7): JusticeCrimeE2eTests 8/8 PASS on live isolated stack** (stages.json; artifact at HEAD via dcda357a6) — REPORT (real CSReportCrimePacket → SCCrimeChanged +10/+10) + MYSQL-CRIME-ROW (id 4096: criminal 1/victim 2/reporter 2/type 3) + MYSQL-CHARACTER (10/10 via MarkDirty setter fix) + RESTART-PERSISTENCE (hard kill → points/rows survive via CrimeManager.Load) + WANTED-SEAM (state=1 from live Wanted-buff check, cp≥50) prove the **report→points→persist→wanted legs: W=scoped-2, A=1** (single live scenario, no negative paths), **R=1** (single restart cycle). **Full-chain W=2 NOT granted** — G1: kill used GM /kill fallback (real ForceAttack+skill damage never landed in safe zone); G2: SCDoodadCreated wire frame unobserved (visibility-driven); G3: wanted via WebApi fallback, SCBuffCreated(3710) unobserved; G4: seam double-applied (cp=145 harness artifact); TrialData.cs:240 stays OPEN under TRIAL-01. |
...
| PRISON-01 | Imprisonment, sentence time, labor/escape/release | M9 | 2 | 2 | U | 1 | U | U | No `PrisonManager` found. **2026-08-25 (mechanics/justice-domain.md):** sentencing + imprisonment live inside `TrialManager` — teleport-to-jail (Marianople Guard Barracks / Isle of Penance) + timed `Prisoner_Nuian`(631)/`Prisoner_Haranyan`(2028) buff for the sentence, offline guilty-time anti-exploit column. **2026-09-12 (90ad756769, justice-domain.md Addendum A3): C=2/W=2/A=1 — graded against the owner-defined contract \"the sentence must persist for its full duration and release on expiry\"; labor/escape tunnels/guard NPCs/cell geometry remain owner-descoped and are NOT claimed by these grades.** RELEASE is data-driven and already existed (the prior \"release-on-expiry logic: ABSENT entirely\" annotation was wrong): canonical `buff_triggers` 190/1128 (`event_id 6` Timeout) → `effects` 3154/11573 → `special_effects` 563/3129 (`special_effect_type_id 25` = `SpecialType.Return`, `value1` 48/262) → `Data/Portal/worldgates.json` \"Nuian Jail Exit\" (zone 183, 11022/12207/134) / \"Haranya Jail Exit\" (zone 141, 16866/8770/149); the real expiry path (`DispelTask` → `Buff.StopEffectTask` → `BuffTrigger.Execute` → `Return.Execute`) teleports the released prisoner to that exit. PERSISTENCE was a real hole and is now FIXED (`Units/Buffs.cs`): `ShouldPersistBuff` refused the sentence buffs (`save_rule_id 1` Normal + `kind_id 2` Bad hit the don't-save-debuffs rule), so a mid-sentence logout silently voided the sentence — no buff, no timer, no release. The sentence is now persisted by a tag-driven exemption (`BuffConstants.TagPrisoner` = tagged_buffs 344; no buff ids hardcoded) and resumes with the REMAINING time (`ComputeRemainingMs` seam; `real_time=0` = timer paused offline), then releases through the same data-driven path. Proven by `AAEmu.UnitTests … PrisonReleaseRigTests` (6 tests on canonical data via real `SkillManager.Load()`/`PortalManager.Load()`): release pre-fix FAIL `Expected to be 11022` / post-fix PASS; persistence pre-fix FAIL (2 tests) / post-fix PASS; ordinary Bad debuff still not persisted; resumed sentence still releases to the canonical exit. Plus a live MySQL round trip (`character_active_buffs` 631 duration=1800000 time_left=719975 → resumed ~12.0 min of a 30 min term, TagPrisoner intact, ordinary Bad 2167 absent) — that DB half was a throwaway probe, not a committed test, so A stays 1 (rig-level) by repo convention (unit rigs must not open MySQL). **H stays U** — no human playthrough; **R/S stay U** (crash-duplication and soak unproven). |
...
| EXPEDITION-01 | Expedition membership, roles, persistence | M9/M10 | U | 2 | U | 2 | U | U | `ExpeditionManager`; organization audit. **2026-08-24 (f8252a37b):** headless verification rig — full lifecycle through the real manager with capture-backed connections: create (party auto-join) → reply-accept outsider → leave → disband; create-without-party refused before mutation. W=2 (real engine path end-to-end); **2026-09-13 LIVE formation PASS:** `ExpeditionFlowE2eTests` — 5 provisioned bots (L10, funded 10k create cost each) form a party (invite/accept x4) then found an expedition through the real `ExpeditionManager` path via the new `ExpeditionCreate` contract action + `ExpeditionFormationScenario` bridge seam; owner-membered + party-auto-joined criteria green, unique name per run; report `/root/aaemu-e2e/logs/expedition-formation-report.json`. **A=2 (formation live; roles/persistence/leave-disband stay rig-level)**; H=UNKNOWN |
...
| AGGRO-PACK-01 | NPC aggro-link packs: shared-pull via `aggro_links`/`npc_aggro_links` membership | Later | U | 1 | U | 1 | U | U | NEW 2026-08-31 (`mechanics/undefined-world-mechanics-2026-08-31.md`): originally truly undefined — `aggro_links` 130 named packs + `npc_aggro_links` 643 rows / 572 distinct NPCs / 126 links / 111 multi-member packs. **2026-09-15 correction (read-only audit): now WIRED** — `NpcGameData.cs:177` loads `npc_aggro_links` into `AggroLinkIdsByNpc` (lookup `SharesAggroLink`); `Behavior.cs:411-416` consults pack membership in `UpdateAggroHelp` (additive: legacy AcceptAggroLink/help-dist/sight/special-rule gates still refuse first); `NpcAggroLinkPackTests` covers the pack contract. W=1/A=1 (live pack-pull proof still open); H=U. Distinct from the catalogued `quest_act_obj_aggro` objectives (PB-002) |
| RESPAWN-LADDER-01 | Data-driven death/resurrection wait ladder (incl. siege ladder + penalty window) | Later | U | 1 | U | 1 | U | U | NEW 2026-08-31 (`mechanics/undefined-world-mechanics-2026-08-31.md`): originally data-only / hardcoded mismatch — `resurrection_waiting_times` 10 rows. **2026-09-15 correction (read-only audit): now WIRED (partial)** — `ResurrectionGameData.cs:52` loads `resurrection_waiting_times` (waiting_time + siege_waiting_time + penalty_duration ladders); `CharacterCombat.cs` consumes it (`ComputeDeathWaitTime` via `GetActiveWaitingTimeSeconds` + `ActivePenaltyWindowSeconds` penalty-window reset); `ResurrectionGameDataTests` covers load/clamp. Siege-waiting-time leg stays unwired by design (no trustworthy is-in-siege signal at the callsite). W=1/A=1; H=U. Refines COMBAT-01 |
...
| PREMIUM-01 | Premium/patron service: purchase premium status granting benefits (labor cap 5000, point accrual, perks) | M9 | U | U | U | U | U | U | NEW 2026-08-25 (census §3#6): PremiumServiceBuy/List/Msg + PayChargeMoney + SCPremiumPoint* wired; wiring surprise: all four premium_* tables ZERO-wired while the premium labor cap is hardcoded in TimedRewardsManager. Own dossier pending. **2026-09-12 correction (read-only sweep):** the *tables* half HOLDS — `premium_benefits`/`_configs`/`_grades`/`_points` still have zero .cs references and no PremiumManager exists. The *hardcoded labor cap* half is STALE: `TimedRewardsManager.cs:31` now reads `AppConfiguration.Instance.Labor.GetCap(isPremium)` (`Configurations.cs:487-488`; values from `Configurations/CharacterSettings.json:47,49-50` — CapLabor 5000 / CapFree 2000 / CapPremium 5000) via `08d2d7a0d` (2026-08-25, ~1.4 h after this row was written; the constants `MaxLabor = 2000` / `MaxLaborPremium = 5000` were still hardcoded at the writing SHA `0ed6d2257`). **The remediation suggested by `zero-wired-domains.md:20` ("drive labor from premium_benefits instead of hardcoded 5000") is therefore partly done via config — do not re-implement it.** |
...
| REPAIR-01 | Repair: restore durability on worn gear, pet items, slave equipment, and placed property | M4 | 2 | 2 | U | 2 | U | U | **2026-09-13 LIVE PASS:** `EquipmentRepairE2eTests` — real GameplayActor.Repair path (Character.DoRepair) at spawned blacksmith 10997: sword 5639 durability 10 → 65; unique account per run, blacksmith template with live spawner, thread itemId through mail repair; report `/root/aaemu-e2e/logs/economy-repair-report.json`. W=2 (real engine path)/A=2 (live proof); H=UNKNOWN |
...
| CINEMA-01 | Cinematics/tutorials: quest-triggered camera scenes with captions/subtitles | M9 | U | U | U | U | U | U | NEW 2026-08-25 (census §3#27): cinemas/cinema_captions/subtitles/effects (long-standing "cinema zero-wired" watch item); Started/CompletedCinema opcodes + CSSaveTutorial. Own dossier pending. **2026-09-12 correction (read-only sweep):** narrow it — the *content tables* are genuinely unread (`cinemas` 149 / `cinema_captions` 97 / `cinema_subtitles` 71 rows; 0 .cs refs each) and `CinemalEffect.Apply` is a bare `Logger.Trace` (`CinemalEffect.cs:17`), but **"no client-triggered cinema flow" is WRONG**: `CSStartedCinemaPacket`/`CSCompletedCinemaPacket`/`CSRequestPermissionToPlayCinemaForDirectingMode` are registered (`GameNetwork.cs:210-212`) and drive real objective credit — `CSStartedCinemaPacket.cs:13` and `CSCompletedCinemaPacket.cs:16` fire `OnCinemaStarted`/`OnCinemaEnded`, which `QuestActObjCinema` subscribes (`:30-34`) and turns into `SetObjective(questAct, 1)` (`:77`). Note `cinema_effects` IS read (`SkillManager.cs:885`), so the "effects" part is imprecise. |
...
| MOULD-01 | Mould/stamp crafting: craft stamp kits to imprint designs onto furniture/paper (UCC-adjacent) | M9 | U | U | U | U | U | U | NEW 2026-08-25 (census §3#33): mould_packs/_pack_items/moulds ZERO-wired (zero-data-wired list); doodad_func_stamp_makers/mould_items/moulds. Own dossier pending. **2026-09-12 correction (read-only sweep):** calmer wording — the three named tables are **data-dead, not unwired**: `mould_packs`, `mould_pack_items` and `moulds` each hold **0 rows** in canonical data, so there is nothing to wire (matches the "data-dead" classification at `zero-wired-domains.md:18`, which recommends skip). Two further errors: `mould_items` **is not a table** at all (`sqlite3` → "no such table"; only `doodad_func_mould_items` exists, 0 rows), and `doodad_func_stamp_makers` **IS read** — `DoodadManager.cs:2401` `SELECT * FROM doodad_func_stamp_makers` → `DoodadFuncStampMaker` (4 rows). |
...
| siege | 5 | 3 | 60% | — |
| loot | 4 | 4 | 100% | — |
| mates | 4 | 4 | 100% | MateIdManager, MateManager |
| merchants | 4 | 2 | 50% | — |
| premium | 4 | 0 | 0% | — |
| ranks | 4 | 0 | 0% | — |
...
> **2026-09-12 domain-table correction (read-only negative-claim sweep).** This table was frozen at its introducing commit `d248a691e` (2026-08-03) and has not been re-derived since; two rows had drifted and are corrected in place above: `siege` 0/5 → **3/5 (60%)** (`DominionManager.LoadSiegeTemplates` reads `siege_zones` :91, `siege_settings` :142, `siege_plans` :166 — `d42e708f5`, 2026-08-26) and `mates` 0/4 → **4/4 (100%)** (all four `mate_equip_*` tables loaded in `MateGameData` — `0482ba3f0`, 2026-08-23; note the zero-data-wired bullet lower in this file was already corrected for the same four tables while this row was not). The `auction` row remains 0/3 for the three name-lookup tables but is misleading without its caveat — see the zero-data-wired bullet. Rows for `premium`, `ranks`, `models`, `world`, `fx-visuals` and `moulds` were re-verified 2026-09-12 and still hold.
...
>
> **2026-09-12 watch-item correction (read-only negative-claim sweep).** Original text preserved above; three of its four items had drifted:
> - **"EtcItemObtain engine no-op" — STALE.** Implemented by `6b2f15a6d` (2026-08-23): `QuestActEtcItemObtain` now `return GetObjective(quest) >= Count;` and credits via `OnItemGather` → `AddObjective(questAct, e.Count)` (the genuine no-op — `return true;` + `return;` — was still present at the writing SHA `6e58e5ac3`).
> - **"honor zero-wired (zero-wired-domains.md)" — WRONG WHEN WRITTEN, and the citation is broken.** Honor was already fully wired at `6e58e5ac3`: `CharacterCombat.cs:129` → `AwardPvpHonor` (:210-289) with the solo/killer/assist math writing `GamePointKind.Honor`, `Character.cs:1594` + :2713 persistence, `ItemManager.cs:1024` `honor_price` load, and the `CSBuyItemsPacket` honor-currency gate/debit (:88-89, :120, :154). Separately, `scorecard-explorations/zero-wired-domains.md` contains **zero** occurrences of "honor" (at HEAD and at the claim SHA) — the parenthetical cites a document that never made this claim, so the intended referent is unrecoverable; it is not re-cited here rather than inventing one.
> - **"cinema zero-wired" — PARTIALLY WRONG.** Content tables are unread, but the client-triggered quest flow exists — see the CINEMA-01 ledger row for the packet/objective path.
> - **"ZoneKill ZoneId unenforced" — HOLDS.** Re-verified: `QuestActObjZoneKill.cs` references `ZoneId` only at the property declaration (:18) and a debug log (:41), never against `OnZoneKillArgs.ZoneGroupId` (`UnitEvents.cs:196`).

## Zero-data-wired domains (data exists, server ignores it)

> **2026-09-12 read-only negative-claim sweep.** This list was authored 2026-08-03 (`d248a691e`) and is updated only by hand, so entries drift in the silent direction — a later reader landing leaves no trace here. All nine bullets below were re-verified against both the C# tree (exact table-name grep, `.worktrees/` excluded) and the canonical `compact.sqlite3` (md5 `78b3bdbf038db3b927056106efdf91af`): **`siege` was stale (3 of 5 tables wired — corrected in place), `auction` and `moulds` were misleading as worded (corrected), and `fx-visuals`, `premium`, `ranks`, `models`, `world` still hold (~0 .cs refs each).** Failures cluster in the STALE direction: the list is mostly right, which is what makes the wrong rows dangerous.

- **fx-visuals** (15 tables): fx_cam_fovs, fx_cgas, fx_cgfs, fx_chrs, fx_decals, fx_group_fx_items...
- **siege** (5 tables): siege_items, siege_plans, siege_settings, siege_ticket_offense_prices, siege_zones — **2026-08-03 snapshot: all 5 unread.** **CORRECTED 2026-09-12 — (3/5 WIRED 2026-08-26, `d42e708f5`):** `DominionManager.LoadSiegeTemplates` reads `siege_zones` (`DominionManager.cs:91`), `siege_settings` (:142) and `siege_plans` (:166, `TableExists`-guarded :190-196); registered `Program.cs:170-171`. `siege_items` (13 rows) and `siege_ticket_offense_prices` (10 rows) remain genuinely unread (0 .cs refs)
...
- **premium** (4 tables): premium_benefits, premium_configs, premium_grades, premium_points — **HOLDS (re-verified 2026-09-12):** all four still have zero .cs references and no PremiumManager exists. Caveat: the related "premium labor cap is hardcoded" claim is **stale** (config-driven since `08d2d7a0d`, 2026-08-25) — see PREMIUM-01 for detail.
- **ranks** (4 tables): rank_reward_links, rank_rewards, rank_scope_links, rank_scopes
- **auction** (3 tables): auction_a_categories, auction_b_categories, auction_c_categories — **HOLDS for the tables, but misleading without precision (2026-09-12).** The three *name-lookup* tables are genuinely unread (0 .cs refs), but the category **semantics are wired**: `items.auction_a/b/c_category_id` (14,734 items carry a non-zero A id) → `ItemManager.cs:1087-1089` `template.AuctionCategoryA/B/C` → live search filtering at `AuctionManager.cs:598-612` inside `SearchAuctionLots` (reached from `CSAuctionSearchPacket.cs:15-20`). The related `GetItemTemplatesForAuctionSearch` (`ItemManager.cs:298`) is **dead** — only test doubles reference it. FK-link reading is heuristic; the code path is exact.
- **models** (3 tables): model_attach_point_strings, model_bindings, model_quest_cameras — **HOLDS (re-verified 2026-09-12):** 0 .cs refs; no indirect reader (`ModelManager` reads only actor_models/ship_models/vehicle_models/models/game_stances, :87/:108/:146/:183/:203). Row counts 81 / 1858 / 100.
- **moulds** (3 tables): mould_pack_items, mould_packs, moulds — **DATA-DEAD, not "ignored" (2026-09-12):** all three hold **0 rows** in canonical data, so there is nothing to wire; classified "data-dead / skip" at `zero-wired-domains.md:18`. (`doodad_func_mould_items` / `doodad_func_moulds` are also 0 rows.)
