...
| SLAVE-01 | Cart/ship summon, seats/cargo, cleanup, recovery | M4 | 1 | 1 | U | U | U | U | `SlaveManager`; vehicle/ship audit; dossier `scorecard-explorations/mechanics/vehicles-ships.md` (canonical 1.2 summon chain = scroll item 15802 SpawnSlave effect → `item_summon_slaves` → slave template; despawn gates 312-range/288-combat/417-packs present in client strings, only 801 packs gate + owner gate implemented; 5–10 min abandonment despawn + 5-min repair wait canonical vs 10-min fork timer; portal/instance despawn per PR #1477; MySQL `slaves` row persists HP/MP, no boot respawn — fork == upstream develop 0-diff on all slave files) |
