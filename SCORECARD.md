...
| CRAFT-01 | Recipe prerequisites, labor/material consume, output | M4 | 1 | 1 | U | U | U | N/A | `CraftManager`; selected pack recipe; dossier `scorecard-explorations/mechanics/crafting.md` (canonical 1.2 recipes = `crafts` 7,010 rows — `skill_id`→`skills.consume_lp` labor, `req_doodad_id` workstation gate 2,366 crafts/94 doodads, `actability_limit` prof. gates to 75k; mats consumed at completion; `rate`/`require_grade`/`main_grade` unenforced; workstation range/type/ownership not validated in `Craft()` — fork == upstream develop) |
