...
public sealed class ActionMcpServer
{
    ...
    private async Task<string> CallToolAsync(JsonNode? id, JsonNode? parameters)
    {
        ...
        var response = name switch
        {
            ...
            "repair" => await _client.PostAsync("/api/actors/repair",
                Body(arguments, "bot", "blacksmithNpcObjId", "itemId", "idempotencyKey")),
            "sell_specialty" => await _client.PostAsync("/api/actors/sell_specialty",
                Body(arguments, "bot", "traderNpcObjId", "idempotencyKey")),
            "trade_offer" => await _client.PostAsync("/api/actors/trade_offer",
                Body(arguments, "bot", "targetCharacterObjId", "idempotencyKey")),
            "trade_putup" => await _client.PostAsync("/api/actors/trade_putup",
                Body(arguments, "bot", "itemTemplateId", "count", "idempotencyKey")),
            "trade_lock_ok" => await _client.PostAsync("/api/actors/trade_lock_ok",
            ...
            "withdraw_item" => await _client.PostAsync("/api/actors/withdraw_item",
                Body(arguments, "bot", "itemTemplateId", "idempotencyKey")),
            "plant" => await _client.PostAsync("/api/actors/plant",
                Body(arguments, "bot", "seedItemTemplateId", "x", "y", "z", "zRot", "scale", "idempotencyKey")),
            "harvest" => await _client.PostAsync("/api/actors/harvest",
                Body(arguments, "bot", "doodadObjId", "idempotencyKey")),
            "craft" => await _client.PostAsync("/api/actors/craft",
                Body(arguments, "bot", "craftId", "doodadObjId", "idempotencyKey")),
            "buy" => await _client.PostAsync("/api/actors/buy",
                Body(arguments, "bot", "merchantNpcObjId", "itemTemplateId", "count", "idempotencyKey")),
            "sell" => await _client.PostAsync("/api/actors/sell",
                Body(arguments, "bot", "merchantNpcObjId", "itemId", "idempotencyKey")),
            "pack_pickup" => await _client.PostAsync("/api/actors/pack_pickup",
                Body(arguments, "bot", "doodadObjId", "idempotencyKey")),
            "put_down" => await _client.PostAsync("/api/actors/put_down",
                Body(arguments, "bot", "packItemTemplateId", "idempotencyKey")),
            "load_pack_onto_vehicle" => await _client.PostAsync("/api/actors/load_pack_onto_vehicle",
                Body(arguments, "bot", "slaveObjId", "placedPackDoodadObjId", "idempotencyKey")),
            "board_vehicle" => await _client.PostAsync("/api/actors/board_vehicle",
                Body(arguments, "bot", "vehicleObjId", "attachPoint", "idempotencyKey")),
            "unboard_vehicle" => await _client.PostAsync("/api/actors/unboard_vehicle",
                Body(arguments, "bot", "idempotencyKey")),
            "drive_vehicle" => await _client.PostAsync("/api/actors/drive_vehicle",
                Body(arguments, "bot", "vehicleObjId", "x", "y", "z", "speed", "timeoutSec", "idempotencyKey")),
            ...
            ["tools"] = new JsonArray(
                ...
                Tool("withdraw_item",
                    ...
                    ObjectSchema(("bot", true, "string"), ("itemTemplateId", true, "number"),
                        ("idempotencyKey", false, "string"))),
                Tool("plant",
                    "Plant a seed or young tree at a world position through the normal placement path. POST /api/actors/plant.",
                    ObjectSchema(("bot", true, "string"), ("seedItemTemplateId", true, "number"),
                        ("x", true, "number"), ("y", true, "number"), ("z", true, "number"),
                        ("zRot", false, "number"), ("scale", false, "number"), ("idempotencyKey", false, "string"))),
                Tool("harvest",
                    "Harvest a mature crop doodad. POST /api/actors/harvest.",
                    ObjectSchema(("bot", true, "string"), ("doodadObjId", true, "number"),
                        ("idempotencyKey", false, "string"))),
                Tool("craft",
                    "Craft ONE recipe step at a workbench doodad through the normal craft engine path. POST /api/actors/craft.",
                    ObjectSchema(("bot", true, "string"), ("craftId", true, "number"),
                        ("doodadObjId", false, "number"), ("idempotencyKey", false, "string"))),
                Tool("buy",
                    "Buy an item from an NPC merchant through the normal store engine path. POST /api/actors/buy.",
                    ObjectSchema(("bot", true, "string"), ("merchantNpcObjId", true, "number"),
                        ("itemTemplateId", true, "number"), ("count", false, "number"), ("idempotencyKey", false, "string"))),
                Tool("sell",
                    "Sell an item to an NPC merchant through the normal store engine path. POST /api/actors/sell.",
                    ObjectSchema(("bot", true, "string"), ("merchantNpcObjId", true, "number"),
                        ("itemId", true, "number"), ("idempotencyKey", false, "string"))),
                Tool("repair",
                    "Repair equipment at a blacksmith NPC. POST /api/actors/repair.",
                    ObjectSchema(("bot", true, "string"), ("blacksmithNpcObjId", true, "number"),
                        ("itemId", false, "number"), ("idempotencyKey", false, "string"))),
                Tool("sell_specialty",
                    "Sell the carried trade pack at a specialty trader. POST /api/actors/sell_specialty.",
                    ObjectSchema(("bot", true, "string"), ("traderNpcObjId", true, "number"),
                        ("idempotencyKey", false, "string"))),
                Tool("trade_offer",
                    "Offer a direct player trade to a character. POST /api/actors/trade_offer.",
                    ObjectSchema(("bot", true, "string"), ("targetCharacterObjId", true, "number"),
                        ("idempotencyKey", false, "string"))),
                Tool("trade_putup",
                    "Put up an item stack into the active trade. POST /api/actors/trade_putup.",
                    ObjectSchema(("bot", true, "string"), ("itemTemplateId", true, "number"),
                        ("count", false, "number"), ("idempotencyKey", false, "string"))),
                Tool("trade_lock_ok",
                    "Lock and confirm the active trade. POST /api/actors/trade_lock_ok.",
                    ObjectSchema(("bot", true, "string"), ("idempotencyKey", false, "string"))),
                Tool("pack_pickup",
                    "Pick up a placed trade pack doodad into the backpack slot. POST /api/actors/pack_pickup.",
                    ObjectSchema(("bot", true, "string"), ("doodadObjId", true, "number"),
                        ("idempotencyKey", false, "string"))),
                Tool("put_down",
                    "Put down a carried trade pack doodad through its use skill. POST /api/actors/put_down.",
                    ObjectSchema(("bot", true, "string"), ("packItemTemplateId", true, "number"),
                        ("idempotencyKey", false, "string"))),
                Tool("load_pack_onto_vehicle",
                    "Load a trade pack onto a vehicle cargo point (snap-to-cargo-point). POST /api/actors/load_pack_onto_vehicle.",
                    ObjectSchema(("bot", true, "string"), ("slaveObjId", true, "number"),
                        ("placedPackDoodadObjId", false, "number"), ("idempotencyKey", false, "string"))),
                Tool("board_vehicle",
                    "Board a vehicle at an attach point (Driver/Passenger/etc). POST /api/actors/board_vehicle.",
                    ObjectSchema(("bot", true, "string"), ("vehicleObjId", true, "number"),
                        ("attachPoint", false, "string"), ("idempotencyKey", false, "string"))),
                Tool("unboard_vehicle",
                    "Unboard the active vehicle. POST /api/actors/unboard_vehicle.",
                    ObjectSchema(("bot", true, "string"), ("idempotencyKey", false, "string"))),
                Tool("drive_vehicle",
                    "Drive a boarded vehicle to a world destination via the vehicle movement model. POST /api/actors/drive_vehicle.",
                    ObjectSchema(("bot", true, "string"), ("vehicleObjId", true, "number"),
                        ("x", true, "number"), ("y", true, "number"), ("z", true, "number"),
                        ("speed", false, "number"), ("timeoutSec", false, "number"), ("idempotencyKey", false, "string"))),
                Tool("accept_quest",
