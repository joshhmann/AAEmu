...
/// 21-30) / t13 (band 31-40) / t14 (band 41-50) / t15 (band 51-55 +
/// lvl-99 straggler 3465) through the scenario harness and writes
...
public class QuestScenarioTierTests
{
    ...
    public async Task TierManifests_DriveAndWriteRunnabilityReport()
    {
        ...
        sb.AppendLine("# Quest Runnability — M1-5 + M2 (wave closures + band 1-55 + lvl-99 straggler census)");
        sb.AppendLine();
        ...
        if (File.Exists(metaPath))
        {
            ...
            // ---- Stragglers (WI-9): top-level quests above the top band get
            // dedicated rows (census-meta "stragglers" key; total=1 each). ----
            if (metaDoc.RootElement.TryGetProperty("stragglers", out var stragglers))
            {
                foreach (var s in stragglers.EnumerateArray())
                {
                    var sQid = s.GetProperty("questId").GetUInt32();
                    var sRows = rows.Where(r => r.QuestId == sQid).ToList();
                    var sPass = sRows.Count(r => r.Verdict == "Pass");
                    var sFail = sRows.Count(r => r.Verdict == "Fail");
                    var sSkip = sRows.Count(r => r.Verdict == "Skip");
                    var sPct = 100.0 * (sPass + sSkip) / 1;
