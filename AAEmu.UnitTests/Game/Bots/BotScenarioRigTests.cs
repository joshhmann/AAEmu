...
public class BotScenarioRigTests
{
    ...
    /// The file is REGENERATED per test process (first call truncates) so a
    /// committed evidence file stays deterministic and gate runs never
    /// churn the tree (same discipline as the census's runnability.md).</summary>
    ...
    private static void AppendEvidence(BotScenarioRunner.ScenarioRunResult result)
    {
        lock (s_evidenceLock)
        {
            var path = Path.Combine(RepoRoot(), "scorecard-explorations", "bot-template-rig.md");
