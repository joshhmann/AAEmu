...
Data claims below corroborated 2026-09-08 via Archaeology MCP
(`aaemu-archaeology` 1.1.0, `query_sql`/`lookup_row`,
source_id `compact.sqlite3`, version `1.2 r208022`,
path `AAEmu.Game/Data/compact.sqlite3`); 5.0-only data
(`compact.server.table.sqlite3`, trimmed pak, GeoDataMode) is
non-transferable. Patch target paths verified against diff `---`/`+++`
headers, resolved to true repo paths (namespace-dot filenames normalized).
...
| npc-cave-float-clamp | 🔶 COMPARE, don't copy | `Models/Game/NPChar/TrackAndStoreCoordinates.cs` + `NpcPathfindingImproved.cs` ABSENT at true paths (verified 2026-09-08) — their fix has no direct seam. Their MaxGroundRise-8m rise-reject vs our ±2m deadband + whitelist + dry-run telemetry (Phase 1 landed). Different philosophy, same cave problem. Needs a compare pass against our height-resolution path. |
...
| quest-act-types-and-disable | 🔶 PARTIAL | `Models/Game/Quests/Acts/QuestActConReportNpcGroup.cs` ABSENT here at true path (verified 2026-09-08); other targets exist — verify each. |
| transfer-path-cell-offset | 🔶 CHECK | Transfer route loading + offsets; our TransferRideE2e passes, so compare carefully. |
| vehicle-oob-movement-oom | 🔶 PARTIAL | `Core/Packets/C2G/CSStartedCinema2Packet.cs` ABSENT here at true path (verified 2026-09-08; `CSStartedCinemaPacket.cs` exists); check `CSMoveUnitPacket` validation + `WorldManager` bounds. |
...
| cinematic-vehicle-dismount | 🔶 PARTIAL | `CSStartedCinema2Packet.cs` ABSENT (verified); `CSStartedCinemaPacket.cs` exists — verify. |
| cinema-directing-mode-freeze | ⛔ 5.0-only | `CSRequestPermissionToPlayCinemaForDirectingModePacket.cs` ABSENT at true path (verified 2026-09-08). |
