...
- Status: OPEN — RESIDUAL: Retribution 2167 SCBuffCreated not detected on either bot wire despite the crime branch executing server-side (SetCriminalState → AddBuff(2167) precedes the evidence step that provably ran). Next probe: dump attacker buff list via bridge charState after AGGRESS + check Buffs.AddBuff early-return paths (StackRule.Refresh silent return, Buffs.cs:424-449) for buff 2167's stack_rule
