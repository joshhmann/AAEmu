...
- Evidence: FULL AAEmu.UnitTests green 2439/2439 post-fix (incl. the 4 new seam tests); instrumented E2E run 04:00 shows damage frames + bloodstain doodad 44294 spawning (crime chain executes); PEACE-BLOCK/HOMELAND-SHIELD stages stay PASS
- Status: OPEN — RESIDUAL NARROWED: compact.sqlite3 (SELECT-only) row 2167 is stack_rule_id=1 (BuffStackRule.Refresh), duration=30000, max_stack=1. The recovered DB-free real-Skill.Use rig seeds an existing assault to avoid persistence-backed bloodstain writes, then observes Retribution present and SCBuffCreated (0x0B6) on first AddBuff; a second Refresh increases the captured SCBuffCreated count. Therefore Buffs.AddBuff does not silently suppress creation on Refresh and no wire/source fix is claimed. The prior live no-frame result remains unmeasured against this detector/fixture; PB-007 stays OPEN pending a live rerun with buff-state dump and corrected packet accounting.
