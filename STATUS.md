...
- **Drift verdict:** deploy unfrozen after the review above (prior frozen state superseded). Open follow-ups: SQL-updates-in-image slice.
- **Gates:** Tier1 `./scripts/gate.sh` 3273 total / 3272 pass / 0 fail / 1 skip (`Provision_Activate_Persist_Deactivate_RoundTrip`, environmental); MCP BotControl smoke 44 tools + archaeology gate smoke 24 tools / 679 tables; `./scripts/archaeology-cycle.sh` 156/156 + 24-tool smoke, compact md5 `78b3bdbf038db3b927056106efdf91af` unchanged; `BotSurveySensesTests` 13/13, `NeedsFarmModuleTests` 39/39; Tier2 `ShipyardFramePersistenceE2eTests` 1/1 + `MovementStopInterruptE2eTests` 1/1 on isolated lanes; bake max 384.5046m corroborated to float32 precision; dashboard tester link `{"ok": true, "server_online": true, "entities": []}` (no players/bots online).
...
- Report `g2-a5-tier3-sixhour-report.json` runAtUtc 2026-09-06T02:49:08Z: `passed: true`, `failures: []`, window FULL 360.00001 min (`windowCompleted` true), 360 samples, RSS growth 25.4 MB (budget 512), SaveP95/Max within budget per report. Tested binary source `02903804b` (= b2 code). Scope: b2 passed AT revision 02903804b, not at current HEAD — commits since are additive/default-off/test-only (E2E bridge ops, farmer slice, seam guard, unit lane) and touch no dormant-timer/tick path, but the letter of the evidence is the tested revision. A5 as a whole stays OPEN pending (a) SHAPE re-show. Run 6h16m05s, 1/1.
