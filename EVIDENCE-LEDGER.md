...
- 2026-09-12 — Exploit-class merchant/auction persistence findings (fixed; automated evidence, not human/client-feel):
  ...
  - **Auction mail attachment container relocation — FIXED, commit SHA pending:** `MailForAuction` previously stamped owner/slot and appended attachments without relocating `_holdingContainer` at cancel (`FinalizeForCancel`), expiry (`FinalizeForFail`), or buyer settlement (`FinalizeForSaleBuyer`, seller-side `AuctionAttachments`). Reboot consequently reloaded the item as `SlotType.Auction` (6) instead of Mail (5), orphaning it. All three sites now use `AttachItemForReturn(item, receiverId)` to resolve the receiver Mail container by owner ID via `ItemManager.GetItemContainerForCharacter` and relocate the same instance, preserving details. The earlier auction test omitted `container_id` and did not reboot after settlement; the live-stack .165 E2E exposed RED (slot 6, one orphan) and GREEN after the fix (slot 5, zero orphans, details blob byte-identical across kill-9).
