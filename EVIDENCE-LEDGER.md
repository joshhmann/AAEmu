...
- 2026-09-08 — A5 CLOSED (runtime): SHAPE 4/4 re-show at engine `a23cdc33e` — report `g2-a5-tier3-report.json` runAtUtc 2026-09-08T08:43:32Z (isolated lane `/root/aaemu-e2e-a5-shape`, 27xx ports, DB 27306, fresh volume, `E2E_REBUILD=1`, 15m02s, probe 1/1): 1000/1000 dormant seeded (SQL-verified), exactly 50 embodied (50/50 dematerialized, 0 after), RSS Δ -12.9% (6204.9 → 5404.7 MB medians, within ±15%), wake-to-visible p95 256.2 ms (p50 221.4, max 277.8; window 33.0 s at default 3/sweep @ 2 s pacing). Conjunction: (a) ✓ at `a23cdc33e`; (b1) soak #2 PASS 2026-09-05 (tested binary `322390b32`, FULL 360.00008-min window, `passed: true`) ✓ historical; (b2) asserted soak PASS 2026-09-06 (`02903804b`) + bounded restart PASS 2026-09-05 ✓ historical — legs stand at their tested revisions per the roadmap letter. En-route: scheduler DI deadlock find + engine fix `a23cdc33e` (first-ever scheduler resolve wedged forever; metrics `tick.available=true` proof on a fresh isolated boot) and test-only FindGamePid stale-cache guard `9b790f69f` (arm-T RSS had read -1/-1/-1). No grade promotion beyond the A5 gate; H unchanged.
- 2026-09-08 — M8 QUALIFIED exit (runtime, .165 day-scale C5 re-soak): report `c5-resoak-day-report.json` (fresh clone @ `a4d35fee8` = develop HEAD, canon assets md5 `78b3bdbf038db3b927056106efdf91af`) — ledger 24/24 cycles PASS (6/6 criteria each; Phase S 173/173); 4/4 PID-verified kill-9 restarts with byte-equality (cold boot + 4 restarts; PIDs 3125422→3128152→3130855→3133248→3135565; MySQL equality at restart #1); scheduler VALID via real lease/wake (+21,616 steps, failed 0); physics/autosave budgets PASS; DB-volume INVALID by design (no scoped counter; deferred, never PASS). One accepted transient exception (not raw green): single 1112 ms ActiveRegionTick at +162 s post-boot, first farm-region activation inside one character tick, mechanism verified at the WorldManager.cs per-character budget-check placement, 0 recurrence in 78 min after, 340+ repeats at zero cost; warmup-blind window unchanged, no code fix. Chain: C4/Crafter/C5 slice commits as landed on develop. No ledger-state-7 claim; M8 exit feel stays UNKNOWN.
- 2026-09-12 — QA-suite validation findings (all fixed; automated/live-stack smoke-tier E2E on the .165 testing server, not human evidence; no ledger state promoted):
  - **LoginNetwork shutdown reconnect storm:** `LoginProtocolHandler.OnDisconnect` unconditionally restarted the Login connection through the `// TODO Hard Restart` path. During shutdown, the async disconnect callback raced DI teardown; `LoginNetwork.Start()` dereferenced the disposed `IServiceProvider`, causing an unhandled `ObjectDisposedException` in a threadpool callback and process death mid-shutdown, plus an approximately 9,000-line `Connection to LoginServer has been lost` storm. Fixed by `a5b7cafb1`: `LoginNetwork._stopping`/`IsStopping`, a Start early return while stopping, idempotent Stop via `Interlocked.Exchange`, and a handler early return with a shutdown log line.
  - **Rowboat E2E log-path drift:** `RowboatE2eTests` read `E2E_ROOT/logs/game.log` (captured server stdout), while NLog writes `<E2E_ROOT>/runtime/game/Logs/Server.log`; therefore PHYSICS-ADDSHIP and NO-LEAKED-PHYSICS-BODY could not observe their evidence channel. `ensure-log-caps.sh` also capped lanes at Info while AddShip/RemoveShip are `Logger.Debug`. Fixed by `5e8d7d73f` to read the NLog file and enable a smoke-only Debug rule. This was not a physics defect; physics was active throughout.
  - **E2E teardown container leak:** the teardown trap failed to remove the lane MySQL container because `kill_lane_procs` returned nonzero under `set -e`, preventing Compose `down`, and a missing closing `}` made the remote heredoc unparseable. `E2eStack` never tears down MySQL (StopAll only kills Login/Game), so the shell trap owns it. Fixed by `5e8d7d73f` with the brace added, Compose unswallowed, and a ports-free assertion.
  - Evidence: smoke-tier Rowboat E2E on the .165 testing server changed LOG-HYGIENE from failure to 0 unhandled/fatal errors. This is automated/live-stack evidence, not a human/client-feel run. The green lane artifact root is `/root/aaemu-dev/soak-artifacts/smoke-rb/20260912-014234`; no additional lane artifact path is asserted here.

- 2026-09-12 — Exploit-class merchant/auction persistence findings (fixed; automated evidence, not human/client-feel):
  - **Merchant buy shop-source bypass — FIXED `8f1d0f263`:** `CSBuyItemsPacket` previously gated `SellsItem` behind `npcObjId != 0`, allowing crafted `npcObjId=0` packets to acquire arbitrary registered templates at `items.price`, labor-free, near any doodad or nothing. The fix makes membership unconditional (`pack == null || !pack.SellsItem(itemId)`) and resolves legitimate doodad shops through `doodad.FuncGroupId` → `DoodadManager.GetFuncsForGroup` → `DoodadFuncStoreUi`/`GetFuncTemplate` → `MerchantPackId` → `NpcManager.GetGoods`, refusing invalid or empty packs. Canonical data has 9 `doodad_func_store_uis` rows / 5 packs, loaded at `DoodadManager.cs:2427-2443`; spawned General Vendor doodad `Id=1400`/`UnitId=7961` carries group `21933`. `MerchantRigTests` is 12/12; stashed pre-fix packet reproduced the exploit in both zero-NPC cases (item `88901` landed), proving the defect. The stale `GameplayActor.Buy` currency comment was corrected in the same change.
  - **Auction mail attachment container relocation — FIXED, commit `242ac6d48`:** `MailForAuction` previously stamped owner/slot and appended attachments without relocating `_holdingContainer` at cancel (`FinalizeForCancel`), expiry (`FinalizeForFail`), or settled buy (`FinalizeForSaleBuyer`, seller-side `AuctionAttachments`). All three sites now route through the single `AttachItemForReturn(item, receiverId)` helper, which resolves the receiver Mail container by owner ID via `ItemManager.GetItemContainerForCharacter` and relocates the same instance, preserving details. The live-stack automated E2E on the .165 host exposed RED at `soak-artifacts/auction-cancel/20260912-025046` (reloaded `slot_type` 6/Auction, one orphan) and GREEN at `soak-artifacts/auction-cancel/20260912-035828` (`slot_type` 5/Mail, zero orphans, details blob byte-identical across kill-9); this is not human/client-feel evidence. The settled-buy variant survived the original audit because its assertion checked `slot_type`/`owner` but never `container_id` and never rebooted after settlement.
- 2026-09-12 — Evidence-method correction (docs-only; no ledger-state promotion): archaeology dated three stale/wrong claims. **PRISON-01 — WRONG WHEN WRITTEN:** `0ed6d2257` (2026-08-25) asserted release-on-expiry was absent, but the release path was already present from `e4df9b291` (2026-07-07), a 49-day lead, through canonical `buff_triggers`/`special_effects` data and worldgates 48/262. **Merchant R1 — STALE BY LATER CHANGE:** the dossier `402042ae7` was written 2026-09-10 03:42, before the `c00090c97` fix at 14:07 (10h25m later). **GameplayActor `&&` comment — STALE BY LATER CHANGE:** `a39d1530` (2026-08-13) preceded the `cb514c42e` gate fix (2026-08-26) by 13 days. Two errors were ordinary drift after later fixes; PRISON-01 was a method error: a negative claim was made from C# (`TrialManager`) alone while the mechanic was wired through data. Review requirement: a claim that a mechanic is ABSENT/unwired/stubbed MUST be evidenced against the canonical data path as well as the code path; check and name the relevant `compact.sqlite3` trigger/effect/func tables (for example `buff_triggers`, `special_effects`, and `*_funcs`) and state that they were empty/unreferenced. “I read the manager and saw nothing” is not sufficient negative evidence. Caveat: `AAEmu.Game/Data/compact.sqlite3` is not tracked in git (`git log -- AAEmu.Game/Data/compact.sqlite3` returns nothing); it exists on disk with md5 `78b3bdbf038db3b927056106efdf91af`, so its data provenance cannot be dated by commit and data-backed negatives are harder to re-verify.

- 2026-09-13 — Six live-stack E2E proofs close the remaining backlog (35/35; automated/proxy evidence, no H claim, no ledger-state promotion beyond the noted A grades):
  - **COMBAT-01 recovery leg:** `RecoveryHealingE2eTests` PASS — honest wound (ReduceCurrentHp) 720→1, potion 8515 (L10, skill 11715, fixed heal 990 = heal_effects row 78 by actual id) through the real `GameplayActor.UseItem` contract path restores 1→991 (+990). Additive mail `wound`/`use`/`stock` bridge ops + charState hp/maxHp observability. Report `/root/aaemu-e2e/logs/recovery-heal-report.json`.
  - **REPAIR live:** `EquipmentRepairE2eTests` PASS — real `GameplayActor.Repair` (Character.DoRepair) at spawned blacksmith 10997: sword 5639 durability 10 → 65. Unique account per run + thread itemId + Completed assert. Report `/root/aaemu-e2e/logs/economy-repair-report.json`. SCORECARD REPAIR-01 → W=2/A=2.
  - **PACK-01 solo pack-sale:** `SpecialtyPackSaleE2eTests` PASS — m8-economy-cycle-v0 hauler=true: pack 26489 crafted (full canonical recipe 4747 x1 + 19887 x3 + 15768 x5 stocked via the ordinary acquisition path) → wagon summoned (scroll 18660) → board → load → drive → unload → SELL-GOLD at gold trader 10664, payout-formula criterion green (mail 124540c == round(base 91238 x ratio x 1.05), pack consumed, labor -60). Needed fixes: missing `HandleEconomyDayCycleScenario` dispatch line (handler existed, unknown-template rejected) + full recipe stock in the hauler rig. Report `/root/aaemu-e2e/logs/specialty-pack-sale-report.json`.
  - **TRADE-01 live handshake:** `DirectTradeE2eTests` PASS — two provisioned bots, bob converged to 2 m (5 m trade range), potato 7992 x1 through OFFER→PUTUP→LOCK-ALICE→LOCK-BOB, alice-conserved + bob-received criteria green. Report `/root/aaemu-e2e/logs/trade-handshake-report.json`. SCORECARD TRADE-01 → A=2 (item-handshake live; currency leg open).
  - **FARM-01 butcher leg:** `LivestockButcherE2eTests` PASS — calf 16225 planted via the real `GameplayActor.Plant` path, E2E-compressed growth (GrowthRate 3600) 5780 → 5781 → 12774 → 5782, butcher skill 13972 via the real `GameplayActor.Interact` → `Doodad.Use` chain (implemented `DoodadFuncButcher`) lands loot tail 9907 with beef 8048 x14 in the bag. Networked fallback added to farm ops; growth poll accepts the transient 12774 phase. Report `/root/aaemu-e2e/logs/agriculture-butcher-report.json`.
  - **EXPEDITION-01 live formation:** `ExpeditionFlowE2eTests` PASS — 5 provisioned bots (L10, funded 10k create cost each) form a party (invite/accept x4) then found an expedition through the real `ExpeditionManager` path via the new `ExpeditionCreate` contract action + `ExpeditionFormationScenario` bridge seam; owner-membered + party-auto-joined criteria green, unique name per run. Report `/root/aaemu-e2e/logs/expedition-formation-report.json`. SCORECARD EXPEDITION-01 → A=2 (formation live; roles/persistence/leave-disband stay rig-level).
  - Evidence basis: working tree at HEAD `c7351b242e4088e0db5ef7926659a948312cb2fb` + uncommitted bot/E2E/bridge changes (binaries republished from source per run); canonical DB md5 `78b3bdbf038db3b927056106efdf91af` unchanged; live runs strictly solo-sequential (parallel E2E on the shared stack raced RestartGameServer); Tier-1 gate green 3130 total / 3129 passed / 0 failed / 1 skipped + MCP 44 + archaeology 24 on clean ports (a stray-E2E-stack-on-1260 `BridgeDarkByDefault` failure was environmental, confirmed 2/2 clean after kill); archaeology-cycle 156/156 + 24-tool smoke green. Runs are scripted-actor/bot-functional evidence only — H stays UNKNOWN everywhere.
