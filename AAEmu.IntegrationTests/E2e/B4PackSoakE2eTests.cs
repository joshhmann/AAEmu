...
/// <summary>
/// B4 PACK-01 S run: budgeted trade-pack full-loop soak
/// (craft → carry → load → sell) with restart persistence.
///
/// STATED BUDGET (this is the S-run contract — re-running the single-pass
/// M4_2/M4Vehicles E2Es without this table does NOT close the card):
///   actors:    2 concurrent bots round 1 (b4pack01/b4pack02 m3a-m4-replay:
///              farm → craft → putdown/pickup → summon → board/BindSlave →
///              load → drive → unboard → SELL (merchant yield, real Sell path)
///              → bank) + 2 fresh bots round 2 in-window (b4pack03/b4pack04
///              m3a-m4-replay = continuous load + second loop proof).
///   sell-leg split (stated): the loop SELL rides the m3a-m4 SELL stage; the
///              specialty-trader SellSpecialty leg is the SpecialtyManagerTests
///              21/21 unit leg. DEFECT FOUND 2026-09-10: the m8-economy-cycle-v0
///              bridge template is dispatch-dead (HandleEconomyDayCycleScenario
///              exists but HandleCommand never routes to it — bridge answers
///              "unknown template"), so no live economy-cycle leg exists until
///              that dispatch lands; this S-run does not depend on it.
///   loops:     2 full PACK loops pre-restart + 2 full loops under measurement.
...
///   restarts:  1 × kill -9 of ONLY the game process (PID-handle kill via
///              E2eStack.RestartGameServer — MySQL + login stay; never pkill),
///              placed BEFORE the measured window (long up-segment rule: no
...
///              never Wake), DB ≤ 500/min/embodied-char (denominator = 5
...
///
/// PASS = every scenario passed AND pre/post kill-9 attachment byte-equality
/// (M51 shape: same slave row, same cargo point, same item link, plant_time
/// not rewritten, local transform intact) AND ledger equality (money/bank/
/// items/containers/mails identical — zero dup, zero loss) AND every budget
/// verdict green. Anchors: SpecialtyManagerTests 21/21 (sale math, 80/20,
/// gates), M4_2TradePackRestartE2eTests (plant_time + made_unit_id survive
/// kill -9), M4ExitIntegratedSessionTests (full chain proof).
///
...
/// Teardown is PID-verified (RestartGameServer PID + StopAll + closed-port
/// probe) — pkill is banned. H stays UNKNOWN: scripted-actor evidence only.
/// </summary>
[Collection("e2e")]
public class B4PackSoakE2eTests
{
    private const string ReplayTemplate = "m3a-m4-replay";
    private static readonly string[] Round1Bots = ["b4pack01", "b4pack02"];
    private static readonly string[] Round2Bots = ["b4pack03", "b4pack04"];

    // Canonical compact.sqlite3 ids (M51/Hauler shape).
    ...
    private static readonly uint[] PackStorageBoxTemplates = [3446, 4893];
    ...
    private sealed record MetricsSample(double TickP95, double TickMax, double RegionMs,
        ...
        double SchedAvg, double SchedMax, bool SchedAvail, double RssMb);
    ...
    [Fact]
    [Trait("Category", "e2e")]
    public async Task B4Pack_FullLoopSoak_OneKill9_ByteEqualLedger_NoDupLoss_WithinBudgets()
    {
        AssertIsolatedLane();
        ...
        {
            // ---- round 1: full loops -------------------------------------------------
            round1["b4pack01"] = RunScenario("b4pack01", 420_000);
            round1["b4pack02"] = RunScenario("b4pack02", 420_000);
            Check(round1["b4pack01"] == "PASS", $"round-1 b4pack01 m3a-m4-replay not PASS: {round1["b4pack01"]}");
            Check(round1["b4pack02"] == "PASS", $"round-1 b4pack02 m3a-m4-replay not PASS: {round1["b4pack02"]}");

            using (var bridge = new BotDriveClient(E2eStack.BridgePort))
                CheckSaveAck(bridge.Call("{\"cmd\":\"save\"}", timeoutMs: 120_000), "pre-kill");

            var preAttach = new Dictionary<string, AttachmentSnapshot>();
            var preLedger = new Dictionary<string, LedgerSnapshot>();
            foreach (var bot in Round1Bots)
            {
                var charId = ResolveCharacterId(bot);
                Check(charId > 0, $"round-1 bot '{bot}' has no characters row");
                if (charId == 0) continue;
                preLedger[bot] = SnapshotLedger(charId);
                CheckLedgerSane(preLedger[bot], bot, "pre-kill");
                preAttach[bot] = SnapshotAttachment(charId);
                CheckAttachmentShape(preAttach[bot], charId, bot, "pre-kill");
                if (_fingerprints.TryGetValue(bot, out var fp) && !FingerprintMatches(preAttach[bot], fp))
                    _failures.Add($"[pre-kill] {bot}: snapshot diverged from immediate post-scenario fingerprint [{fp}] | whereabouts: {PackWhereabouts(charId)}");
            }

            // ---- the ONE kill -9 (outside the measured window) ------------------------
            killedPid = E2eStack.RestartGameServer();
            Check(killedPid > 0, "no game PID was killed — the restart cannot be trusted");

            foreach (var bot in Round1Bots)
            {
                var charId = ResolveCharacterId(bot);
                Check(charId > 0, $"post-kill bot '{bot}' has no characters row");
                if (charId == 0 || !preLedger.TryGetValue(bot, out var pre)) continue;
                var post = SnapshotLedger(charId);
                CheckLedgerEqual(pre, post, bot, "R1");
                if (preAttach.TryGetValue(bot, out var preA))
                    CheckRestartIntact(preA, SnapshotAttachment(charId), charId, bot, "R1");
            }
            ...
            // Round 2 = continuous load inside the window.
            var t1 = Task.Run(() => RunScenario("b4pack03", 420_000));
            var t2 = Task.Run(() => RunScenario("b4pack04", 420_000));
            ...
            round2["b4pack03"] = await t1;
            round2["b4pack04"] = await t2;
            Check(round2["b4pack03"] == "PASS", $"round-2 b4pack03 m3a-m4-replay not PASS: {round2["b4pack03"]}");
            Check(round2["b4pack04"] == "PASS", $"round-2 b4pack04 m3a-m4-replay not PASS: {round2["b4pack04"]}");
            ...
            foreach (var bot in Round2Bots)
            {
                ...
                if (charId > 0) CheckLedgerSane(SnapshotLedger(charId), bot, "window-end");
            }
            var round2Attach = SnapshotAttachment(ResolveCharacterId("b4pack03"));
            CheckAttachmentShape(round2Attach, ResolveCharacterId("b4pack03"), "b4pack03", "window-end");
            ...
            var last = samples[^1];
            var firstSteps = samples[0].StepsRun;
            var stepsDelta = last.StepsRun - firstSteps;
        ...
        finally
        {
            ...
            catch (Exception ex) { Console.WriteLine($"[b4pack] cleanup failed (non-fatal): {ex.Message}"); }
        }

        teardown = VerifyTeardown();
        await WriteReportAsync(startedAt, budgets, killedPid, round1, round2, verdicts,
            windowWallMin, steadyMin, warmupExcludedPhysics, warmupExcludedOverruns, bootTimes, teardown);
        Assert.True(_failures.Count == 0,
            $"B4 PACK S-run RED ({_failures.Count}):\n- {string.Join("\n- ", _failures)}");
    }
    ...
    private static void AssertIsolatedLane()
    {
        Assert.False(string.Equals(E2eStack.E2eRoot, "/root/aaemu-e2e", StringComparison.Ordinal),
            "B4 PACK S-run refuses the shared default E2E_ROOT (isolated lane required)");
    ...
    private string RunScenario(string bot, int timeoutMs)
    {
        ...
        {
            using var bridge = new BotDriveClient(E2eStack.BridgePort);
            var req = $"{{\"cmd\":\"scenario\",\"template\":\"{ReplayTemplate}\",\"bot\":\"{bot}\",\"fresh\":true}}";
            var resp = bridge.Call(req, timeoutMs: timeoutMs);
            ...
            Console.WriteLine($"[b4pack] scenario {ReplayTemplate} bot {bot}: {(passed ? "PASS" : $"FAIL @{stage}: {reason}")} stages={_scenarioStages[bot]}");
            if (passed)
                _fingerprints[bot] = FingerprintAttachment(bot);
    ...
    /// <summary>Immediate post-scenario attachment fingerprint — discriminates a
    /// scenario end-state branch from a world-idle/save race when the pre-kill
    /// snapshot diverges.</summary>
    private static string FingerprintAttachment(string bot)
    {
        try
        {
            var charId = ResolveCharacterId(bot);
            if (charId == 0) return "|no-char-row|";
            var snap = SnapshotAttachment(charId);
            var pack = snap.Bindings.Count(b => b.ItemId > 0);
            var item = snap.Bindings.Where(b => b.ItemId > 0).Select(b => b.ItemId).FirstOrDefault();
            return $"|slave={(snap.Slave == null ? "none" : snap.Slave.Id.ToString())}|bindings={snap.Bindings.Count}|packBindings={pack}|packItem={item}|";
        }
        catch (Exception ex) { return $"|fingerprint-error:{ex.GetType().Name}|"; }
    }

    private static bool FingerprintMatches(AttachmentSnapshot snap, string fp)
        => fp.Contains($"|bindings={snap.Bindings.Count}|") &&
           fp.Contains($"|packBindings={snap.Bindings.Count(b => b.ItemId > 0)}|");

    /// <summary>Pack-template whereabouts for a character: slave rows, slave-owned
    /// binding rows, and every 26488 item row with its container — the definitive
    /// dump when the cargo shape diverges.</summary>
    private static string PackWhereabouts(uint charId)
    {
        try
        {
            ...
            {
                cmd.CommandText = "SELECT COUNT(*), GROUP_CONCAT(id) FROM slaves WHERE summoner = @c";
                cmd.Parameters.AddWithValue("@c", charId);
                using var r = cmd.ExecuteReader();
                r.Read();
                parts.Add($"slaveRows={r.GetInt64(0)} ids={r.GetString(1)}");
            }
            using (var cmd = conn.CreateCommand())
            {
                cmd.CommandText = "SELECT COUNT(*), SUM(item_id > 0) FROM doodads WHERE owner_type = 2 AND house_id IN (SELECT id FROM slaves WHERE summoner = @c)";
                cmd.Parameters.AddWithValue("@c", charId);
                using var r = cmd.ExecuteReader();
                r.Read();
                parts.Add($"slaveBindings={r.GetInt64(0)} withItems={r.GetInt64(1)}");
            }
            using (var cmd = conn.CreateCommand())
            {
                cmd.CommandText = "SELECT id, container_id, slot_type, slot, count FROM items WHERE owner = @c AND template_id = 26488 ORDER BY id";
                cmd.Parameters.AddWithValue("@c", charId);
                using var r = cmd.ExecuteReader();
                var rows = new List<string>();
                while (r.Read()) rows.Add($"{r.GetUInt64(0)}:c{r.GetUInt64(1)}/st{r.GetInt32(2)}/s{r.GetInt32(3)}/n{r.GetInt32(4)}");
                parts.Add($"pack26488=[{string.Join(";", rows)}]");
            }
            return string.Join(" ", parts);
        }
        catch (Exception ex) { return $"whereabouts-error: {ex.GetType().Name}: {ex.Message}"; }
    }
    ...
    private void CheckAttachmentShape(AttachmentSnapshot snap, uint charId, string bot, string phase)
    {
        ...
        Check(slave.OwnerType == 0u && slave.OwnerId == charId && slave.Summoner == charId,
            $"[{phase}] {bot}: slave ownership link broken (owner_type {slave.OwnerType} owner {slave.OwnerId} summoner {slave.Summoner} char {charId})");
        Check(slave.ItemId > 0, $"[{phase}] {bot}: wagon slave row lost its summoning-item link");
        ...
        if (packRows.Count == 1)
        {
            ...
            Check(binding.OwnerType == 2u && binding.HouseId == slave.Id,
                $"[{phase}] {bot}: binding not keyed to this slave");
            ...
            Check(binding.TemplateId == PlacedPackDoodadTemplateId && binding.CurrentPhaseId == PlacedPackStartPhaseId,
                $"[{phase}] {bot}: placed-pack template/phase {binding.TemplateId}/{binding.CurrentPhaseId} != 6068/15677");
            ...
            Check(snap.Items.TryGetValue(binding.ItemId, out var item) && item.TemplateId == PackItemTemplateId && item.Count == 1,
                $"[{phase}] {bot}: pack item row missing/wrong for binding item {binding.ItemId}");
        }
    }
    ...
    private void CheckRestartIntact(AttachmentSnapshot pre, AttachmentSnapshot post, uint charId, string bot, string tag)
    {
        ...
        Check(a.ItemId == b.ItemId, $"[{tag}] {bot}: summoning-item link changed");
    ...
    private static MetricsSample SampleOnce(BotDriveClient bridge)
    {
        ...
        return new MetricsSample(tickP95, tickMax, regionMs, saveP95, saveMax, saveAvail,
            stepsRun, stepsFailed, schedAvg, schedMax, schedAvail, -1);
    }
    ...
    private async Task WriteReportAsync(DateTime startedAt, GateBudgets budgets, int killedPid,
        ...
        double wallMin, double steadyMin, long exclPhys, long exclOver, List<string> boots, string teardown)
    {
        ...
        var report = new
        {
            slice = "B4 PACK-01 S run — budgeted trade-pack full-loop soak (craft→carry→load→sell)",
            card = "PACK-01/SLAVE-01 S runs need STATED budgets (actors × duration × restarts) with measured results",
            stated_budget = new
            {
                actors = "2 round-1 (b4pack01/02 m3a-m4-replay) + 2 round-2 in-window (b4pack03/04 m3a-m4-replay)",
                loops = "2 full PACK loops pre-restart + 2 full loops under measurement",
                duration = $"window ≥ {MinWindowMinutes} min wall, ≥ {MinSteadyMinutes} min steady-state; measured {wallMin:F1} wall / {steadyMin:F1} steady",
                restarts = $"1 × kill -9 game-only (PID-handle, pre-window); killedPid={killedPid}",
            ...
            teardown,
            anchors = "SpecialtyManagerTests 21/21; M4_2TradePackRestartE2eTests; M4ExitIntegratedSessionTests chain; GateBudgetEvaluator defaults; SoakWarmup blinds",
            proxy_note = "scripted-actor / bot-functional evidence — H (feel) stays UNKNOWN"
        };
        await File.WriteAllTextAsync(Path.Combine(EvidenceDir, "b4-pack-soak-report.json"),
            JsonSerializer.Serialize(report, new JsonSerializerOptions { WriteIndented = true }));
        var md = new System.Text.StringBuilder();
        md.AppendLine("# B4 PACK-01 S run — budgeted trade-pack full-loop soak");
        md.AppendLine();
        md.AppendLine($"- SHA: {E2eStack.SourceRevision} · lane: {E2eStack.E2eRoot} · compose: {E2eStack.ComposeProject} · DB: {E2eStack.DbPort}");
        md.AppendLine($"- Verdict: {(report.verdict)} · elapsed: {report.elapsed_seconds:F0}s · R1 killedPid: {killedPid}");
        ...
        await File.WriteAllTextAsync(Path.Combine(EvidenceDir, "b4-pack-soak-report.md"), md.ToString());
    }
    ...
    private static async Task CleanupAsync(string[] bots)
    {
        ...
        catch (Exception e) { Console.WriteLine($"[b4pack] cleanup failed (non-fatal): {e.Message}"); }
    }
}
