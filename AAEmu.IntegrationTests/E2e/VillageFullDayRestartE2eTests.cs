...
/// <summary>
/// M8 C5 soak exit slice — single kill-9 restart mid-FULL-day with equality
/// (the user-approved soak-readiness hook): TWO villagers (farmer + crafter)
/// run one FULL day through the slice-4 composer
/// (<see cref="VillageFullDayCycle"/>, via the live bridge seam) on an
/// ISOLATED LOCAL lane, then the game process takes a real kill-9 restart
/// and the persisted state must be EQUAL pre/post: schedule anchors +
/// lastPhase + profession (B4 playerbot_metadata rows, asserted directly
/// like B4BotRestartPersistenceE2eTests) + inventory + ledger (M51
/// byte-equality style + the EconomyDayCycle money/bank/item reconciliation,
/// now including the evening merchant refund banked in money2) + the
/// B4-flushed M5 audit trail (playerbot_audit rows covering the day AND the
/// evening legs).
///
/// Flow: cold boot (restart first, the M51 warm-merchant mitigation) →
/// bridge scenario m8-village-fullday-s4 (fresh rows, PASS + all criteria) →
...
///
/// ISOLATED LOCAL LANE (hard rules — local short run only, NEVER .165):
///   - E2E_ROOT=/root/aaemu-e2e-soak (dedicated root, own runtime/logs/pids)
///   - Shifted ports: E2E_LOGIN_PORT=3237 E2E_INTERNAL_PORT=3234
///     E2E_GAME_PORT=3239 E2E_STREAM_PORT=3250 E2E_BRIDGE_PORT=3260
///     (E2E_WEBAPI_PORT=3280 when needed)
///   - Own database: DB_HOST_PORT/E2E_DB_PORT=35306 +
///     COMPOSE_PROJECT_NAME=aaemu_e2e_soak (own MySQL container + volume)
///   - Local only: everything on 127.0.0.1; E2E_GAME_HOST is NEVER set
///     (no .165); the day-scale run reserves .165.
///   - Teardown is PID-verified: the killed PID must be non-zero, the old
///     PID must be dead post-restart (verified via /proc cmdline+cwd, the
///     A5 SHAPE probe method — immune to PID reuse), and a NEW game PID
///     under this lane's root must answer the bridge.
///   - NEVER pkill: the ONLY kill path is E2eStack's PID-handle
///     StopGameServer/RestartGameServer. A silent no-kill must never
///     masquerade as a clean restart (killedPid == 0 fails the test).
///
/// Evidence JSON lands in $E2E_ROOT/logs/village-fullday-restart-report.json
/// (lane ports, PIDs, SHA, ledger + schedule verdicts).
///
/// H stays UNKNOWN: proxy/bot-functional evidence only.
/// </summary>
[Collection("e2e")]
public class VillageFullDayRestartE2eTests
{
    private const string FarmerBot = "m8vffarmer";
    private const string CrafterBot = "m8vfcrafter";
    ...
    [Fact]
    [Trait("Category", "e2e")]
    public async Task VillageFullDay_ScheduleProfessionInventoryLedger_SurviveKill9_Equal()
    {
        var startedAt = DateTime.UtcNow;
        var cycleId = $"m8v5-c5s2-{startedAt:yyyyMMdd-HHmmss}";
        E2eStack.EnsureUp();

        // Cold-world contract (M51 §1): scripted walk/merchant legs only
        // complete synchronously on a FRESH boot. Restart first, like B4.
        // The cold-boot kill is PID-verified too — a silent no-kill here
        // would leave a warm world behind.
        var coldKilledPid = E2eStack.RestartGameServer();
        Assert.True(coldKilledPid > 0,
            "cold-boot restart killed no game process — warm-world state would poison the run");
        ...
        try
        {
            ...
            using (var bridge = new BotDriveClient(E2eStack.BridgePort))
            {
                response = bridge.Call(
                    $"{{\"cmd\":\"scenario\",\"template\":\"{VillageFullDayCycle.ScenarioName}\"," +
                ...
                Assert.True(scenarioPassed,
                    $"village full-day FAILED at {failStage}: {failReason}\nEvidence:\n{evidenceText}");
                ...
                // The evening half must have run, not held: both villagers
                // sold through the merchant and returned home.
                foreach (var villager in response.GetProperty("villagers").EnumerateArray())
                {
                    var name = villager.GetProperty("name").GetString();
                    Assert.True(villager.GetProperty("eveningMarketComplete").GetBoolean(),
                        $"villager {name}: evening market did not complete\nEvidence:\n{evidenceText}");
                    Assert.True(villager.GetProperty("eveningReturnedHome").GetBoolean(),
                        $"villager {name}: evening return-home did not complete\nEvidence:\n{evidenceText}");
                    Assert.True(villager.GetProperty("eveningSold").GetInt32() > 0,
                        $"villager {name}: evening sold nothing\nEvidence:\n{evidenceText}");
                }
            ...
            // envelope shape as the half-day handler: villagers[] + ledgers[]).
            ...
            foreach (var (characterId, exp) in expected)
            {
                ...
                Assert.True(exp.TraceCount > 0, $"bot {characterId}: full day must emit trace records");
                Assert.Equal(exp.TraceCount, preAudit[characterId].Count);
                Assert.NotNull(preChars[characterId]);
            }

            // --------------------------------------------------- 5. KILL -9
            // StopGameServer kills the process tree via its PID handle (never
            // pkill) — only MySQL survives. The returned PID proves a real
            // kill happened.
            var killedPid = E2eStack.RestartGameServer();
            Assert.True(killedPid > 0,
                "restart killed no game process — a silent no-kill must never masquerade as a clean restart");

            // PID-verified teardown (A5 SHAPE probe method — /proc cmdline +
            // lane-root cwd, immune to PID reuse): the old game PID is dead…
            Assert.True(IsGamePidDead(killedPid),
                $"old game PID {killedPid} still alive after kill-9 — teardown NOT verified");

            // …and a NEW game process under this lane's root answers.
            var newPid = FindGamePid();
            Assert.NotNull(newPid);
            Assert.NotEqual(killedPid, newPid);
            ...
            foreach (var (characterId, exp) in expected)
            {
                ...
                // (bankMoney carries the evening merchant refund).
            ...
            // -- schedule fidelity post-restart: the reloaded rows still
            // carry each villager's profession + anchors + lastPhase (the
            // projection the day-scale run diffs, not just byte equality).
            foreach (var (characterId, exp) in expected)
            {
                var postMeta = SnapshotMetadata(characterId);
                Assert.NotNull(postMeta);
                Assert.Equal(exp.Profession, postMeta!.Profession);
                Assert.Contains($"\"lastPhase\":\"{exp.LastPhase}\"", postMeta.Schedule);
            }
        }
        finally
        {
    ...
    private sealed record BotExpectation(string Profession, string LastPhase, int TraceCount, long EveningRefund,
    ...
    /// profession. Mirrors the half-day save-window poll, extended to the
    /// full day's two bots + audit + metadata.
    /// </summary>
    ...
    // ----------------------------------------------- PID-verified teardown
    // (A5 SHAPE probe method: /proc cmdline + lane-root cwd — immune to PID
    // reuse; a bare Process.GetProcessById liveness check is NOT enough).

    /// <summary>Live game PID under this lane's root (null when absent).</summary>
    ...
    /// <summary>True when the PID is no longer OUR game process (dead, or the
    /// PID was reused by a foreign process).</summary>
    private static bool IsGamePidDead(int pid)
    {
        try
        {
            return !IsGameProcess(pid, Path.GetFullPath(E2eStack.E2eRoot));
        }
        catch
        {
            return true;
    ...
    private async Task WriteReportAsync(DateTime startedAt, string cycleId, bool scenarioPassed,
        string failStage, string failReason, string evidenceText, int coldKilledPid)
    {
        Directory.CreateDirectory(EvidenceDir);
        var report = new
        {
            slice = "M8 C5 soak exit — single kill-9 restart mid-FULL-day with equality",
            card = "soak-readiness hook: 2 villagers, 1 full day, 1 restart, schedule/profession/inventory/ledger equality",
            path = "live bridge seam: HandleVillageFullDayScenario → VillageFullDayCycle.Run (slice-4 composer, unchanged)",
            scenario = VillageFullDayCycle.ScenarioName,
            cycleId,
            bots = new[] { FarmerBot, CrafterBot },
            lane = new
            {
                e2e_root = E2eStack.E2eRoot,
                compose_project = E2eStack.ComposeProject,
                login_port = E2eStack.LoginPort,
                internal_port = E2eStack.InternalPort,
                game_port = E2eStack.GamePort,
                stream_port = E2eStack.StreamPort,
                bridge_port = E2eStack.BridgePort,
                db_port = E2eStack.DbPort,
                game_host = "127.0.0.1 (local lane only — .165 reserved for the day-scale run)",
            },
            ...
            teardown = new
            {
                cold_boot_killed_pid = coldKilledPid,
                kill = "PID-handle only via E2eStack.RestartGameServer (NEVER pkill)",
                old_pid_dead_verified = true,
            ...
            asserted_rows = new[]
            {
                ...
                "characters: id/account/name/level/money/money2 (incl. evening merchant refund)/world + position (M51 epsilon) — equal",
        ...
        await File.WriteAllTextAsync(Path.Combine(EvidenceDir, "village-fullday-restart-report.json"),
    ...
    private static async Task CleanupAsync()
    {
        ...
        {
            Console.WriteLine($"[village-fullday-restart] cleanup failed (non-fatal): {e.Message}");
        }
    }
