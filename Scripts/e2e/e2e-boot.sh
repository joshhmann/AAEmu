...
# timeout; stale AAEmu instances are killed by preflight (never boot into an
# occupied port — 2026-08-07 hardening), non-AAEmu foreign holders hard-fail
# with the offending pids; >1GiB logs are truncated (disk-fill guard);
# runtime layout is assembled to match the E2E test runner (E2eStack.cs)
# byte for byte. Teardown + byte-identical baseline proof: e2e-reset.sh.
set -euo pipefail
